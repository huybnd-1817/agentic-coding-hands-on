...
final class KudosViewModelTests: XCTestCase {
    ...
    // MARK: - Fire badge computation

    func testShowFireBadge_activeBonusWithMultiplier_true() async {
        let baseDate = Date(timeIntervalSince1970: 0)
        let bonus = EventBonus(
            id: UUID(),
            startsAt: baseDate,
            endsAt: baseDate.addingTimeInterval(1000),
            multiplier: 2,
            label: "Double Hearts"
        )
        ...
        repo.bonusBehavior = .success(bonus)
                let vm = makeVM(repo: repo, clock: { baseDate.addingTimeInterval(500) })

        await vm.reload()

        XCTAssertTrue(vm.showFireBadge)
    }

    func testShowFireBadge_activeBonusMultiplierOne_false() async {
        let baseDate = Date(timeIntervalSince1970: 0)
        let bonus = EventBonus(
            id: UUID(),
            startsAt: baseDate,
        ...
        XCTAssertFalse(vm.showFireBadge)
    }

    func testShowFireBadge_expiredBonus_false() async {
        let baseDate = Date(timeIntervalSince1970: 0)
        let bonus = EventBonus(
            id: UUID(),
            startsAt: baseDate,
            endsAt: baseDate.addingTimeInterval(100),
            multiplier: 2,
            label: "Expired"
        )
        ...
        let repo = KudosRepositoryFake()
        repo.bonusBehavior = .success(bonus)
                let vm = makeVM(repo: repo, clock: { baseDate.addingTimeInterval(200) })  // After endsAt
    ...
    func testShowFireBadge_noBonus_false() async {
        let repo = KudosRepositoryFake()
